...
func TestSandboxDetachStopsWait(t *testing.T) {
...
func TestSandboxExecDetachStopsWait(t *testing.T) {
	...
	proc, err := sb.Exec(ctx, []string{"sleep", "10"}, nil)
	g.Expect(err).ToNot(gomega.HaveOccurred())
...
func TestSandboxDetachStopsAllStreams(t *testing.T) {
	...
	command := []string{"bash",
		"-c",
		"for i in {1..30}; do date +%T; sleep 1.0; done"}

	sb, err := tc.Sandboxes.Create(ctx, app, image, &modal.SandboxCreateParams{Command: command})
	g.Expect(err).ToNot(gomega.HaveOccurred())
	...
	proc, err := sb.Exec(ctx, command, nil)
	g.Expect(err).ToNot(gomega.HaveOccurred())
