...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to lookup or create app: %w", err)
	}

	image, err := app.ImageFromRegistry("python:3.13-slim")
	if err != nil {
		log.Fatalf("Failed to create image from registry: %w", err)
	}

	sb, err := app.CreateSandbox(image, modal.SandboxOptions{})
	if err != nil {
		log.Fatalf("Failed to create sandbox: %w", err)
	}
...
for i in range(50000):
	...
	if err != nil {
		log.Fatalf("Failed to execute command in sandbox: %w", err)
	}

	contentStdout, err := io.ReadAll(p.Stdout)
	if err != nil {
		log.Fatalf("Failed to read stdout: %w", err)
	}
	contentStderr, err := io.ReadAll(p.Stderr)
	if err != nil {
		log.Fatalf("Failed to read stderr: %w", err)
	}

	log.Printf("Got %d bytes stdout and %d bytes stderr\n", len(contentStdout), len(contentStderr))
	returnCode, err := p.Wait()
	if err != nil {
		log.Fatalf("Failed to wait for process completion: %w", err)
	}
