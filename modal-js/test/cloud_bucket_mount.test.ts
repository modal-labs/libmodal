...
test("CloudBucketMount constructor with minimal options", () => {
  ...
  expect(mount.bucketType).toBe(CloudBucketMount_BucketType.S3);
...
test("CloudBucketMount constructor with all options", () => {
  ...
  expect(mount.bucketType).toBe(CloudBucketMount_BucketType.R2);
...
test("CloudBucketMount bucket type detection from endpoint URLs", () => {
  const s3Mount = new CloudBucketMount("my-bucket");
  expect(s3Mount.bucketType).toBe(CloudBucketMount_BucketType.S3);

  const r2Mount = new CloudBucketMount("my-bucket", {
    bucketEndpointUrl: "https://my-bucket.r2.cloudflarestorage.com",
  });
  expect(r2Mount.bucketType).toBe(CloudBucketMount_BucketType.R2);

  const gcpMount = new CloudBucketMount("my-bucket", {
    bucketEndpointUrl: "https://storage.googleapis.com/my-bucket",
  });
  expect(gcpMount.bucketType).toBe(CloudBucketMount_BucketType.GCP);

  const unknownMount = new CloudBucketMount("my-bucket", {
    bucketEndpointUrl: "https://unknown-endpoint.com/my-bucket",
  });
  expect(unknownMount.bucketType).toBe(CloudBucketMount_BucketType.S3);
});
