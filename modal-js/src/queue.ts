...
export class Queue_ {
  ...
  // Prevent the constructor from being called directly.
  private constructor(objectId: string) {
    this.objectId = objectId;
  }

  static fromName(name: string): Queue_ {
    return new Queue_(name);
  }

  static async ephemeral(): Promise<Queue_> {
    const resp = await client.queueCreate({
      queueId: "ephemeral",
    });
    return new Queue_(resp.queueId);
  }
